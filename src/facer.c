...
static int __init gaming_kbbl_cdev_init(void)
{
	...
	if (IS_ERR(gkbbl_dev_class)) {
		...
		pr_err("Char drive class creation for gaming keyboard backlight failed: %d\n", err);
...
static void gaming_kbbl_cdev_exit(void)
{
	if (gkbbl_dev_class && !IS_ERR(gkbbl_dev_class)) {
...
static int __init gaming_kbbl_static_cdev_init(void)
{
	...
	if (IS_ERR(gkbbl_static_dev_class)) {
		...
		pr_err("Char drive class creation for gaming keyboard static backlight failed: %d\n", err);
...
static void gaming_kbbl_static_cdev_exit(void)
{
	if (gkbbl_static_dev_class && !IS_ERR(gkbbl_static_dev_class)) {
...
static void acer_platform_profile_remove(void)
{
	if (platform_profile_support) {
#if !RTLNX_VER_MIN(6, 14, 0)
		platform_profile_unregister(&platform_profile_handler);
#endif
...
static int __init acer_wmi_init(void)
{
	...
	if (wmi_has_guid(WMID_GUID3)) {
		...
		if (wmi_has_guid(WMID_GUID4)) {
			...
			err = gaming_kbbl_cdev_init();
			if (err)
				return err;
			err = gaming_kbbl_static_cdev_init();
			if (err) {
